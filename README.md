...
`demo_app.py` is a separate, lightweight Streamlit app that replays
precomputed results instead of running YOLO/Gemini live — meant for hosting
somewhere without a GPU (e.g. Hugging Face Spaces). It only imports
`streamlit`, `pandas`, and `plotly` (see `requirements-demo.txt`).
